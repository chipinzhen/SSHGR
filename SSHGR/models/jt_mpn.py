...
from data.mol_tree import BatchMolGraph, moltree2graph, MolTree
from features import get_atom_fdim, get_bond_fdim


class JunctionTreeGraphEncoder(nn.Module):
    ...
    def __init__(self, args: Namespace, atom_fdim: int, bond_fdim: int):
        ...
        """
        super(JunctionTreeGraphEncoder, self).__init__()
...
class JunctionTreeGraphNN(nn.Module):
    """A message passing neural network for encoding a molecule."""
    ...
    def __init__(self,
        ...
        """
        super(JunctionTreeGraphNN, self).__init__()
        self.args = args
        self.atom_fdim = atom_fdim or get_atom_fdim(args)
        self.bond_fdim = bond_fdim or get_bond_fdim(args) + (not args.atom_messages) * self.atom_fdim
        self.graph_input = graph_input
        self.encoder = JunctionTreeGraphEncoder(self.args, self.atom_fdim, self.bond_fdim)
    ...
    def forward(self,
        ...
        if not self.graph_input:  # if features only, batch won't even be used
            batch = moltree2graph(batch, self.args)
