...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='datachem/ZhangDDI_train.csv')
    parser.add_argument('--separate_val_path', type=str, default='datachem/ZhangDDI_valid.csv')
    parser.add_argument('--separate_test_path', type=str, default='datachem/ZhangDDI_test.csv')
    ...
    parser.add_argument('--graph_encoder', type=str, default='dmpnn', choices=['dmpnn', 'ggnn', 'pretrained_gin', 'pretrained_gcn', 'pretrained_graphsage'])
    ...
    parser.add_argument('--pretrained_gnn_path', type=str, default='./saved_model/bace/gin_contextpred_runseed_0.pt', help='The path of the pretrained gnn.')
    parser.add_argument('--best_score_path', type=str, default=None, help='help to record the best score in files.')
    parser.add_argument('--no_pretrain', default=False, help='setting for baselines')

    parser.add_argument('--seed', type=int, default=10, help='set the seed to reconstruct the experiment.')
    parser.add_argument('--graph_pair', default=False, help='whether to use hierachycal graphs')
    parser.add_argument('--get_initial_drugpair_embedding', default=False, help='whether to get initial drug pair embedding')
    parser.add_argument('--get_final_drugpair_embedding', default=False, help='whether to get final drug pair embedding')
    parser.add_argument('--get_initial_drugpair_embedding_save_file', type=str,  default='./embeddings/initial', help='path to save initial drug pair embedding')
    parser.add_argument('--get_final_drugpair_embedding_save_file', type=str,  default='./embeddings/final', help='path to save final drug pair embedding')
...
# create the edge from the DDI csv file to generate the inter-view graph. Return "edges" and "edges false"
...
def main():
    print(args.pretrained_gnn_path)
    ...
    if args.get_initial_drugpair_embedding:
        model.eval()
        preds, feat, embed, re_feat, re_embed, DGI_loss = model(features, adj_norm, adj_tensor, drug_nums)
        adj_tmp = adj_tensor.cpu().detach().numpy().astype(int)
        embedding = embed.cpu().detach().numpy()
        ...
        for i in range(adj_tmp.shape[0]):
            emb1 = embedding[i]
            for j in range(adj_tmp.shape[1]):
                if i != j:
                    emb2 = embedding[j]
                    if adj_tmp[i][j] == 1:
                        pos_list.append(np.concatenate((emb1, emb2)))
                        # print(np.concatenate((emb1, emb2)).shape)
                    else:
                        neg_list.append(np.concatenate((emb1, emb2)))

        pos_list = np.array(pos_list)
        neg_list = np.array(neg_list)


        np.save(args.get_initial_drugpair_embedding_save_file + '_pos', pos_list)
        np.save(args.get_initial_drugpair_embedding_save_file + '_neg', neg_list)

        # print(pos_list.shape)
        # print(neg_list.shape)
    ...
    if args.best_score_path is not None:
        with open(args.best_score_path, 'w') as f:
            f.write('Test ROC score:' + str(roc_score) + '\n')
            f.write('Test AP score:' + str(ap_score) + '\n')
            f.write('Test F1 score:' + str(f1_score) + '\n')
            f.write('Test ACC score:' + str(acc_score))


    if args.get_final_drugpair_embedding:
        model.eval()
        preds, feat, embed, re_feat, re_embed, DGI_loss = model(features, adj_norm, adj_tensor, drug_nums)
        adj_tmp = adj_tensor.cpu().detach().numpy().astype(int)
        embedding = embed.cpu().detach().numpy()
        ...
        np.save(args.get_final_drugpair_embedding_save_file + '_pos', pos_list)
        np.save(args.get_final_drugpair_embedding_save_file + '_neg', neg_list)
